...
const naveInimigas = () => {
  const modeloIndex = Math.floor(Math.random() * modelosNaves.length);
  const modeloNave = modelosNaves[modeloIndex];
  const inimigo = document.createElement("div");
  inimigo.className = "inimigo";
  inimigo.style.position = "absolute";
  inimigo.setAttribute("data-vida", 5);
  inimigo.style.width = "250px";
  inimigo.style.height = "250px";
  inimigo.style.backgroundImage = "url(" + modeloNave + ")";
...
const moveNaveInimigas = () => {
  ...
  navesInimigas.forEach((naveInimiga) => {
    let posicaoTopNaveInimiga = naveInimiga.offsetTop;
    posicaoTopNaveInimiga += velocidadeNaveInimigas;
    naveInimiga.style.top = posicaoTopNaveInimiga + "px";
    if (posicaoTopNaveInimiga > alturaCenario) {
      vidaAtual -= 5;
      vida.textContent = `Vida: ${vidaAtual}`;
      explosaoNaveInimigaDestruida(naveInimiga.offsetLeft);
      if (vidaAtual <= 0) {
        gameOver();
      }
      naveInimiga.remove();
...
const explosaoNaveInimigaDestruida = (posicaoLeftNaveInimiga) => {
  ...
  explosaoNaveInimiga.style.backgroundImage = "url(explosao1.gif)";
...
const gameOver = () => {
  ...
  perdeu.innerHTML = "Assim como este jogo foi programado, suas emoções também são influenciadas. </br> Não se apegue às falhas e desapontamentos; em vez disso, cultive crenças edificantes e aprecie o mundo ao seu redor!";
